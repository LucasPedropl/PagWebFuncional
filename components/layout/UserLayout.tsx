      ...
      >
        ...
        {isEmpresa && (
           <div className={`px-4 pt-4 pb-2 transition-all duration-300 relative ${isCollapsed ? 'px-2 pt-4' : ''}`}>
              ...
              >
                 {isCollapsed ? (
                    // Modo Colapsado: Apenas a Foto ou Iniciais direto no botão
                    (userProfile?.fotoPerfilPath || user?.fotoPerfilPath) ? (
                       ...
                       <span>{getInitials(userProfile?.nome || user?.nome)}</span>
                    )
                 ) : (
                    // Modo Expandido: Card completo com Foto, Textos e Setas
                    <>
                       <div className="flex items-center gap-3 overflow-hidden">
                          ...
                          <div className="flex flex-col text-left overflow-hidden transition-all duration-300">
                             <span className="text-sm font-bold text-slate-900 truncate">{userProfile?.nome || user?.nome || 'Cliente'}</span>
                             <span className="text-xs text-blue-600 font-medium truncate">Área do Cliente</span>
                          </div>
                       </div>
